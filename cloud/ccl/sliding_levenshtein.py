...
def sliding_levenshtein_raw(s1, s2):
	if len(s1) > len(s2):
		s1, s2 = s2, s1
...
def sliding_levenshtein_v2(s1, s2, threshold=0.25, MIN_QUERY_STRING_LENGTH=4):
	...
	if (len(this_query_string_processed) < MIN_QUERY_STRING_LENGTH):
		...
		if (len(this_page_name_processed) < len(this_query_string_processed)):
			return (sliding_levenshtein_raw(this_page_name_processed, this_query_string_processed) <= dynamic_levenshtein_threshold(this_query_string_processed, this_query_string_processed, threshold))
		else:
			for i in range(len(this_page_name_processed)-len(this_query_string_processed)+1):
				if (sliding_levenshtein_raw(this_page_name_processed[i:i+len(smaller_string)], this_query_string_processed) <= dynamic_levenshtein_threshold(this_query_string_processed, this_query_string_processed, threshold)):
					return True
			return False
...
'''
def sliding_levenshtein_pct(s1, s2, MIN_QUERY_STRING_LENGTH=4):
	larger_string = (s1 if (len(s2) < len(s1)) else s2)
	smaller_string = (s2 if (len(s2) < len(s1)) else s1)
	...
	if (len(smaller_string) < MIN_QUERY_STRING_LENGTH):
		...
		levenshtein_results = [sliding_levenshtein_raw(x, smaller_string) for x in slides]
		return ((1 - (min(levenshtein_results) / len(smaller_string))) * (len(smaller_string) / len(larger_string)))

'''
	Returns similarities at offsets of larger string
'''
def sliding_levenshtein_pct_annotated(s1, s2, MIN_QUERY_STRING_LENGTH=4):
	larger_string = (s1 if (len(s2) < len(s1)) else s2)
	smaller_string = (s2 if (len(s2) < len(s1)) else s1)
	larger_string = larger_string.lower()
	smaller_string = smaller_string.lower()
	if (len(smaller_string) < MIN_QUERY_STRING_LENGTH):
		...
		levenshtein_results = [(x, smaller_string) for x in slides]
		levenshtein_results = [sliding_levenshtein_raw(x, smaller_string) for x in slides]
		return [((1 - (x / len(smaller_string))) * (len(smaller_string) / len(larger_string))) for x in levenshtein_results]